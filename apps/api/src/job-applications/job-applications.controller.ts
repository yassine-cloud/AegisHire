...
export class JobApplicationsController {
  ...
  async checkIfApplied(
    ...
    const applied = await this.jobApplicationsService.checkIfApplied(
      jobId,
      user.id,
    );
    return { applied };
  ...
  @ApiOkResponse({
    description: 'Returns application with related job and company data',
  })
  async getApplication(
    @CurrentUser() user: SupabaseJwtPayload,
    @Param('applicationId') applicationId: string,
  ): Promise<any> {
    return this.jobApplicationsService.getApplicationWithJob(
      applicationId,
      user.id,
    );
  }
  ...
  async updateApplication(
    ...
    return this.jobApplicationsService.updateApplication(
      applicationId,
      user.id,
      dto,
    );
  ...
  async deleteApplication(
    ...
    return this.jobApplicationsService.deleteApplication(
      applicationId,
      user.id,
    );
  }
